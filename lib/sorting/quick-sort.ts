 ...
 * @returns
 */
export const naivePartition = (arr: number[], low: number, high: number, p: number): void => {
    const temp = []
 ...
 */
export const lomutoPartition = (arr: number[], low: number, high: number, p?: number): number => {
 ...
 */
export const hoaresPartition = (arr: number[], low: number, high: number): number => {
    // console.info("before: ", arr)
    const pivot = arr[low]
    let i = low - 1;
    let j = high + 1;
...
export const quickSortLomuto = (arr: number[], low: number, high: number): void => {
    if (low >= high) return
 ...
 */
export const quickSortHoares = (arr: number[], low: number, high: number): void => {
    if (low >= high) return
